...
and three held keys — and never a position, a battery level or a catch; all three are refused with
`SERVER_AUTHORITY`. Its `shared/` is a **byte-for-byte copy** of the cabinet's pure layer (UMD modules
imported for the globals they set), regenerated by `node tools/mirror-sim.mjs` in
`javascript-games/games/hide-and-seek` and guarded by `mirror.test.mjs` against a manifest committed
in both repos. The demons are not server-side yet — `endRoundByDemon` is wired to the round's one
ending, waiting on the cabinet to pull the demon's navigation out of its renderer.
