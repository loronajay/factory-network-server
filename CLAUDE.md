...
Both refuse any client-authored outcome with `SERVER_AUTHORITY`.

`hide-and-seek` is the first game here whose match is a **continuously ticking world** rather
than a sequence of turns: one interval per lobby (2-8 players) advances a mirrored 60hz simulation of
a four-floor hotel and publishes a snapshot at 15hz. Clients send **inputs** — a direction, a facing
and four held keys — and never a position, a battery level, a door state or a catch; all of those are
refused with `SERVER_AUTHORITY`. Its `shared/` is a **byte-for-byte copy** of the cabinet's pure layer
(UMD modules imported for the globals they set), regenerated by `node tools/mirror-sim.mjs` in
`javascript-games/games/hide-and-seek` and guarded by `mirror.test.mjs` against a manifest committed
in both repos.

The whole hotel is authoritative: **two demons** hunt inside the tick (`demon-logic.js` in the mirror
— the client's copy is a puppet posed from the snapshot), and every door, secret panel, drawer, key
ring, the elevator and each dropped flashlight battery is contested state resolved here
(`fixtures-logic.js`). Interaction is **edge-triggered** — a client's `interact` flag is a request,
and the fixture it operates is picked server-side by distance, height and facing, never named by the
